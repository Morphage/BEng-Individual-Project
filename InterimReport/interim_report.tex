...
\section{Objectives}
Having identified the core problem, we were lead to formulating objectives in order to make the project successful and useful to the parties involved. \newline
...
\item \textbf{Automated generation -} There should be a mechanism to allow for some degree of automated generation of exercises. This will provide a large supply of ``fresh" questions, so that students don't end up answering the same questions and memorizing the answers to them.
\end{itemize}

While investigating existing solutions (Section 2.3 - Related Work) we found out that "web questions" and progress tracking were available in most, whereas adaptive difficulty and especially automated generation weren't as present. 
